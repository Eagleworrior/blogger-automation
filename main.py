...
def get_news_articles():
    ...
    try:
        ...
        for item in data.get('articles', [])[:5]:
            if item['title'] == '[Removed]' or not item['content'] or not item['url']: continue
            html_content = f"<p><i>Source: {item['source']['name']}</i></p><p>{item['description'] or ''}</p><p>{item['content']}</p><p><a href='{item['url']}' target='_blank'>Read more</a></p>"
            articles.append({"title": item['title'], "content": html_content, "url": item['url']})
        return articles
    ...
    for blog_id in BLOGGER_IDS:
        ...
        for article in news_articles:
            ...
            try:
                ...
                print(f"❌ Error: {error}")
                if error.resp.status == 429: time.sleep(60)
